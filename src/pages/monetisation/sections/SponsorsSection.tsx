...
import { surfacesAtPlanCreation } from '@/utils/offers'
...
const blankSponsor: SponsorDraft = {
  name: '', tradingName: '', category: '', description: '',
...
/** Green "LIVE" pill shown when a partner passes every go-live condition. */
...
const SponsorsSection = () => {
  ...
  const update = (id: number, status: SponsorStatus, verb: string) => {
    ...
    audit('Partners', `${verb} partner`, s.name)
  ...
  const handleSave = (data: SponsorDraft, id?: number) => {
    if (id) {
      ...
      audit('Partners', 'Edited partner', data.name)
      ...
      audit('Partners', 'Added partner', sponsor.name)
  ...
  return (
    <div>
      <SectionTitle title="Partners" subtitle="Manage full partner profiles, safety review, package assignment and status." action={<PrimaryButton Icon={Plus} label="Add Partner" onClick={() => canEdit && setCreating(true)} />} />

      <Card>
        <DataTable headers={['PARTNER', 'CATEGORY', 'VAT NUMBER', 'PACKAGE', 'STATUS', 'SAFETY', 'LIVE', 'ACTIONS']}>
          {sponsors.map((s) => (
            <tr key={s.id} className="hover:bg-surface-subtle transition-colors duration-150">
              <Td>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-semibold text-ink-primary">{s.tradingName || s.name}</div>
                    <div className="text-xs text-ink-muted">{s.city || '—'}</div>
                  </div>
                </div>
              </Td>
              <Td><Badge text={s.category || '—'} bg={colors.bgInput} color={colors.textSecondary} /></Td>
              <Td className="tabular-nums">{s.vatNumber || <span className="text-ink-muted">—</span>}</Td>
      ...
      <div className="fixed inset-0 z-[101] flex items-center justify-center p-5 pointer-events-none">
        <div className="bg-surface rounded-[20px] w-full max-w-[640px] max-h-[92vh] flex flex-col pointer-events-auto shadow-modal animate-[modalSlide_0.25s_ease]">
          <div className="py-[22px] px-7 flex justify-between items-center shrink-0">
            <h2 className="text-xl font-bold text-ink-primary m-0">{initial ? 'Edit Partner' : 'Add Partner'}</h2>
          ...
          <div className="flex-1 overflow-auto px-7 pb-5">
            ...
            <div className="grid grid-cols-2 gap-[14px]">
              <FieldWithLabel label="Registered business name"><PillInput value={form.name} onChange={(v) => set('name', v)} placeholder="e.g. BrewHaus Coffee Ltd" /></FieldWithLabel>
              <FieldWithLabel label="Trading name"><PillInput value={form.tradingName} onChange={(v) => set('tradingName', v)} placeholder="e.g. BrewHaus Coffee" /></FieldWithLabel>
            </div>
            ...
            <div className="grid grid-cols-2 gap-[14px]">
              <FieldWithLabel label="VAT number"><PillInput value={form.vatNumber} onChange={(v) => set('vatNumber', v)} placeholder="e.g. GB 123 4567 89" /></FieldWithLabel>
              <FieldWithLabel label="Company number (optional)"><PillInput value={form.companyNumber ?? ''} onChange={(v) => set('companyNumber', v)} placeholder="e.g. 09847215" /></FieldWithLabel>
            </div>
            ...
            <FieldWithLabel label="Partnership package">
              <SelectPill
            ...
            <SectionHeading>Offers &amp; perks</SectionHeading>
            <LinkedOffers partnerId={initial?.id} />

            <SectionHeading>Workflow</SectionHeading>
            <div className="grid grid-cols-2 gap-[14px]">
              <FieldWithLabel label="Partner status"><SelectPill value={form.status} onChange={(v) => set('status', v as SponsorStatus)} options={[...SPONSOR_STATUSES]} /></FieldWithLabel>
            ...
            <div className={`rounded-2xl p-4 border ${eligibility.isLive ? 'border-success bg-success-light/40' : 'border-line-light bg-surface-input'}`}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[13px] text-ink-secondary">
                  {eligibility.isLive ? 'All conditions met — this partner is live.' : `${eligibility.blockers.length} condition(s) still blocking go-live.`}
                </span>
              </div>
          ...
          <div className="py-[18px] px-7 border-t border-line-light flex gap-[10px] justify-end shrink-0">
            ...
            <PrimaryButton label={initial ? 'Update Partner' : 'Add Partner'} onClick={() => form.name.trim() && onSubmit(form)} />
          </div>
        </div>
      </div>
    </>
...
/** Read-only roll-up of the offers this partner funds, edited over in Offers. */
const LinkedOffers = ({ partnerId }: { partnerId?: number }) => {
  const { offers } = useMonetisation()
  const linked = partnerId === undefined ? [] : offers.filter((o) => o.partnerId === partnerId)

  if (linked.length === 0) {
    return (
      <div className="rounded-2xl bg-surface-input p-4 mb-[14px] text-[13px] text-ink-secondary">
        {partnerId === undefined
          ? 'Save the partner first, then attach discount codes and perks from the Offers section.'
          : 'No offers yet. Add discount codes, special offers or venue perks from the Offers section.'}
      </div>
    )
  }

  return (
    <div className="rounded-2xl bg-surface-input p-4 mb-[14px] flex flex-col gap-3">
      {linked.map((o) => (
        <div key={o.id} className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="font-mono text-[13px] font-bold text-ink-primary">{o.code}</span>
              <Badge text={o.kind} bg={colors.bgInput} color={colors.textSecondary} />
              {surfacesAtPlanCreation(o) && <Badge text="In plan creation" bg={colors.successLight} color={colors.successText} />}
            </div>
            <div className="text-xs text-ink-muted mt-0.5">{o.venue || 'All venues'} · {o.description}</div>
          </div>
          <div className="text-[13px] font-semibold text-ink-primary shrink-0">{o.value}</div>
        </div>
      ))}
    </div>
  )
